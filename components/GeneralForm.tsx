...
export const GeneralForm: React.FC<GeneralFormProps> = React.memo(({
  profile,
  handleProfileChange,
    ...
    <>
      ...
      <fieldset>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            >
              <option value=""></option>
              {genTranslations.occupationOptions.map(option => (
                <option key={option.value} value={option.value}>
                  {option.label}
                </option>
              ))}
      ...
      <fieldset>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {genTranslations.concernOptions.map(option => {
            const isSelected = profile.concerns?.includes(option.value);
            return (
              <button
                type="button"
                key={option.value}
                onClick={() => handleMultiSelectChange('concerns', option.value)}
                disabled={isLoading}
                aria-pressed={isSelected}
                className={`text-left p-2 rounded-lg border-2 transition-all text-sm disabled:opacity-70 ${
                  isSelected
      ...
      <fieldset>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {genTranslations.selfAssessmentOptions.map(option => {
            const isSelected = profile.selfAssessment?.includes(option.value);
            return (
              <button
                type="button"
                key={option.value}
                onClick={() => handleMultiSelectChange('selfAssessment', option.value)}
                disabled={isLoading}
                aria-pressed={isSelected}
                className={`text-left p-2 rounded-lg border-2 transition-all text-sm disabled:opacity-70 ${
                  isSelected
      ...
      <fieldset>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder={genTranslations.medicationsPlaceholder}
            disabled={isLoading}
          />
        </div>
        ...
        <div className="mt-4">
          ...
          <textarea
            ...
            onChange={handleProfileChange}
            className={textareaClass}
            placeholder={genTranslations.allergiesPlaceholder}
            disabled={isLoading}
          />
        </div>
      </fieldset>
