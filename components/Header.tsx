        ...
        <div className="flex items-center justify-between flex-wrap gap-y-3">
          <div className="flex items-center justify-center space-x-2 sm:space-x-3 min-w-0">
              ...
              <div className="min-w-0">
                  ...
                  <p className="text-xs text-sky-600 -mt-1 hidden sm:block">{headerTranslations.tagline}</p>
              </div>
          </div>

          <nav className="hidden md:flex items-center justify-center gap-2" aria-label="Main navigation">
              <NavButton view="compendium" activeView={activeView} onClick={handleNavigate} icon={<BookOpenIcon className="w-5 h-5"/>} label={navTranslations.compendium} />
              <NavButton view="manual" activeView={activeView} onClick={handleNavigate} icon={<HelpCircleIcon className="w-5 h-5"/>} label={navTranslations.manual} />
          </nav>

          <div className="flex items-center space-x-2 flex-shrink-0">
              <div className="flex items-center space-x-1 p-0.5 bg-slate-100/80 rounded-full border border-slate-200" role="group" aria-label={headerTranslations.fontSize.label}>
                  <ToggleButton isActive={fontSize === 'standard'} onClick={() => handleFontSizeChange('standard')} isSmall>{headerTranslations.fontSize.standard}</ToggleButton>
                  <ToggleButton isActive={fontSize === 'large'} onClick={() => handleFontSizeChange('large')} isSmall>{headerTranslations.fontSize.large}</ToggleButton>
              </div>
              <div className="flex items-center space-x-1 p-0.5 bg-slate-100/80 rounded-full border border-slate-200" role="group" aria-label="Language selection">
